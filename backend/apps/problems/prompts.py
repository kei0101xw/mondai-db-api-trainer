...
- 毎回、新規で創意工夫のある題材を選択してください。同じジャンルやよくある題材の繰り返しは避けてください。
...
以下のような様々なジャンルから、毎回異なる新しい題材を生成してください。
同じ題材の繰り返しを避け、多様性のある問題生成を心がけてください。

## コミュニケーション・SNS系
- SNSアプリ（投稿、いいね、フォロー、メッセージ機能）
- チャットアプリ（グループチャット、メッセージ検索、既読管理）
- 掲示板サービス（スレッド、返信、モデレーション機能）
- 匿名質問箱（質問投稿、回答、評価機能）

## Eコマース・マーケットプレイス系
- ECサイト（商品、カート、注文、レビュー機能）
- フリマアプリ（出品、購入、メッセージ機能）
- グルメサイト（レストラン、予約、レビュー機能）
- ライブコマース（ライブ配信、商品販売、コメント機能）

## タスク・プロジェクト管理系
- タスク管理アプリ（プロジェクト、タスク、マイルストーン、割り当て）
- 時間追跡アプリ（プロジェクト、タスク、時間ログ、レポート）
- 進捗管理ツール（スプリント、ストーリー、バーンダウン）

## コンテンツ・メディア系
- ブログシステム（記事、カテゴリ、タグ、コメント機能）
- ポッドキャストプラットフォーム（エピソード、リスナー、購読、再生履歴）
- ビデオストリーミングサービス（動画、プレイリスト、視聴履歴、推奨機能）
- ニュースアプリ（記事、カテゴリ、購読、ブックマーク機能）

## ファイナンス・金融系
- 家計管理アプリ（口座、取引、カテゴリー、予算管理）
- 送金・決済アプリ（ユーザー、ウォレット、取引、手数料管理）
- 投資アプリ（ポートフォリオ、株価、取引履歴、アラート機能）

## ヘルス・フィットネス系
- フィットネストラッキング（ユーザー、ワークアウト、進捗、目標設定）
- 医療予約システム（医者、診療科目、予約、カルテ管理）
- 食事管理アプリ（食事記録、カロリー、栄養成分、目標管理）
- メンタルヘルスアプリ（セッション予約、カウンセリング記録、リソース）

## 教育・学習系
- オンライン学習プラットフォーム（コース、講義、レッスン、進捗追跡）
- クイズアプリ（問題セット、問題、スコア、ランキング）
- 言語学習アプリ（レッスン、語彙、進捗、スピーキング練習）

## 旅行・地図・位置情報系
- 旅行計画アプリ（旅程、アクティビティ、予算、タイムライン）
- ホテル予約サイト（ホテル、予約、レビュー、支払い）
- ライドシェアアプリ（乗客、ドライバー、マッチング、支払い、評価）

## 不動産・ホテル系
- 不動産仲介サイト（物件、内見予約、購入希望者、相談記録）
- Airbnb風民泊サイト（リスティング、予約、ゲスト管理、レビュー）

## 事業管理・SaaS系
- CRM（顧客、営業機会、営業ステージ、フォローアップ）
- HRM（従業員、給与、勤務表、休暇申請）
- インベントリ管理（商品、在庫、仕入、出荷）

## IoT・スマートホーム系
- IoTデバイス管理（デバイス、ステータス、ログ、アラート）
- スマートホーム（デバイス、シーン、自動化、エネルギー管理）

## ゲーム・エンターテイメント系
- ゲームランキング（ゲーム、プレイヤー、スコア、ランキング）
- イベントチケット販売（イベント、チケット、購入、座席管理）
- 映画館チケット（映画、上映スケジュール、チケット、座席予約）

## その他ユニークな題材
- 図書館管理システム（本、貸出、予約、延滞管理）
- 企業内ナレッジベース（ドキュメント、検索、アクセス権限、更新履歴）
- ペットケア予約（ペット、獣医、予約、医療記録）
- サブスクリプション管理（サブスク、課金、キャンセル、更新）

**重要：上記はあくまで例です。これらの例に捕われず、さらに創意工夫のある新しい題材を積極的に考案してください。**

# 出力形式例（JSON）
必ず以下のJSON形式で出力してください。JSONのみを出力し、それ以外の説明文は含めないでください。
以下はあくまで形式例です。具体的な問題文や模範解答は要件に基づいて生成してください。
...
## 模範解答の品質要件
- model_answerは省略せず、完全な内容を記述してください
- DB設計の模範解答: 完全なCREATE TABLE文、インデックス定義、制約をすべて含めてください
- API設計の模範解答: 完全なAPI設計情報（エンドポイント、認証、入出力、エラー）と実装可能な疑似コードを含めてください
- 「...」や「省略」などの記号を使用せず、実際に動作する設計内容を記述してください
- 疑似コードは関数の中身も含め、実装方針が分かるレベルの詳細度で記述してください
"""
